...
if ($Standard -and $Core -and $Default) {
    ...
    if ($env:MAILOZAURR_DEVELOPMENT) {
        $DevelopmentFramework = if ($PSEdition -eq 'Core') { 'net8.0' } else { 'net472' }
        $DevelopmentLibRoot = [IO.Path]::Combine($PSScriptRoot, 'Sources', 'Mailozaurr.PowerShell', 'bin', 'Debug', $DevelopmentFramework)
        $DevelopmentAssemblyPath = [IO.Path]::Combine($DevelopmentLibRoot, $Library)
        if (Test-Path -LiteralPath $DevelopmentAssemblyPath) {
            $DevelopmentMode = $true
            $Framework = $DevelopmentFramework
            $FrameworkNet = $DevelopmentFramework
        } else {
            Write-Error -Message "No packaged assemblies found and development assembly '$DevelopmentAssemblyPath' was not found"
            return
        }
    } else {
...
try {
    ...
    $ModuleAssemblyPath = if ($DevelopmentMode) {
        [IO.Path]::Combine($DevelopmentLibRoot, $Library)
    } else {
        [IO.Path]::Combine($PSScriptRoot, 'Lib', $LibFolder, $Library)
    }
    $PowerForgeLibDirectory = if ($DevelopmentMode) {
        $DevelopmentLibRoot
    } else {
    ...
    if ($PSEdition -eq 'Core') {
        ...
        if ($DevelopmentMode -and -not (Test-Path -LiteralPath $LoaderAssemblyPath)) {

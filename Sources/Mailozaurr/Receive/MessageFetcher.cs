...
public static class MessageFetcher {
    ...
    public static async IAsyncEnumerable<ImapEmailMessage> Fetch(
        ...
        await mailFolder.OpenAsync(delete ? FolderAccess.ReadWrite : FolderAccess.ReadOnly);
        ...
        foreach (var uid in uids) {
            MimeMessage msg;
            try {
                msg = await mailFolder.GetMessageAsync(uid);
            } catch (Exception ex) {
    ...
    public static async IAsyncEnumerable<Pop3EmailMessage> Fetch(
        ...
        for (int i = 0; i < client.Count; i++) {
            MimeMessage message;
            try {
                message = await client.GetMessageAsync(i);
            } catch (Exception ex) {

...
/// <para type="description">Writes a <see cref="MailFileMessage"/> to a file. The destination extension selects EML, MSG, or TNEF, so the same command handles conversion in either direction.</para>
/// <example>
...
public sealed class CmdletExportMailFile : PSCmdlet {
    ...
    protected override void ProcessRecord() {
        if (InputObject == null || string.IsNullOrWhiteSpace(OutputPath)) return;
        ...
        try {
            result = InputObject.Save(outputPath);
        } catch (Exception exception) {

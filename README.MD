...
The archive records the selected IDs, effective profile settings, and provider mailbox scope, writes EML files in `eml/`, and keeps per-message SHA-256 records in `.mailozaurr-archive/`. Repeating the command with the same inventory verifies each retained file before skipping it, retries missing or altered files, and reports failed IDs. IMAP resumes require the same folder UIDVALIDITY. Graph and Gmail resumes verify the selected mailbox through their identity endpoints; if that lookup is denied or the identity changes, the archive stops. POP3 has no account identity endpoint, so its resume scope binds to the credential used to authenticate; changing that credential requires a new archive directory, even if the account is the same. A changed profile also requires a new archive directory. The command accepts more than 1,000 IDs by processing bounded export batches. The ID file is the caller's selected inventory; this command does not discover every message in a mailbox. IMAP retrieves messages in bounded partial fetches. MailKit buffers each POP3 message before returning its stream, with transfer progress enforcing the configured size limit. Graph and Gmail provider APIs currently return buffered MIME payloads.

...
## Choosing a .NET package (3.0 and later)

Install `Mailozaurr` when the application needs the complete Mailozaurr library surface. It contains the reusable profile, routing, draft, queue, and message-action workflows and brings the four implementation packages together without adding CLI, MCP, PowerShell, or OfficeIMO.Security hosting dependencies:

```shell
dotnet add package Mailozaurr
```

Applications with a narrower job can avoid unrelated provider stacks:

| Use case | Package | What it avoids |
| --- | --- | --- |
| SMTP, IMAP, POP3, MIME, SendGrid, Mailgun, or SES | `Mailozaurr.Internet` | Microsoft and Google auth; OfficeIMO |
| Microsoft Graph mail | `Mailozaurr.MicrosoftGraph` | Google auth; OfficeIMO |
| Gmail API mail | `Mailozaurr.Gmail` | Microsoft auth; OfficeIMO |
| `EmailDocument` and `MimeMessage` interoperability, EML/MSG/OFT/TNEF | `Mailozaurr.Artifacts` | Internet protocols and provider auth |
| Local email and Outlook artifacts without MimeKit | `OfficeIMO.Email` | Every Mailozaurr package |

Graph and Gmail depend on `Mailozaurr.Internet`, so those packages also include the common MIME and protocol foundation. `Mailozaurr.Artifacts` is independent of Internet and depends only on MimeKit and OfficeIMO.Email. Add `OfficeIMO.Security` explicitly in C# when artifact verification or decryption needs its concrete provider.

The PowerShell module remains one batteries-included installation. It consumes the complete `Mailozaurr` package and includes OfficeIMO.Security; PowerShell users do not select NuGet leaves.
...
## PST, OST, OLM, Mbox, EMLX, and mailbox directories

Mailozaurr exposes OfficeIMO.Email's offline store workflows without maintaining a second archive engine. `Import-MailData` detects PST, supported PST-compatible OST data, Outlook for Mac OLM, Mbox, EMLX, Maildir, Apple Mail trees, and directories containing EML files. The returned object owns the open store session, so close it when the workflow is complete:

```powershell
$data = Import-MailData './offline.ost'
try {
    $data.Kind
    $data.Store.Format

    $folders = $data | Get-MailStoreFolder
    $messages = $data | Search-MailStore -SubjectContains 'invoice' -MaxResults 50
    $content = $data | Search-MailStore -Term 'renewal','contract' -ResultsOnly

    $batch = $data | Search-MailStore -Term 'project' -MaxItemsScanned 100
    if (-not $batch.IsComplete) {
        $nextBatch = $data | Search-MailStore -Term 'project' -MaxItemsScanned 100 -ResumeFrom $batch.NextCheckpoint
    }

    $data | Test-MailStore -VerifyStructuralIntegrity
    Export-MailStore -InputObject $data -OutputPath './exported-mail' -Format Eml
} finally {
    $data | Close-MailData
}
```

`Get-MailStoreItem` returns lightweight references by default. Add `-Read` to project selected content while the store remains open, and use `-Parts` when only metadata, bodies, recipients, or attachment metadata is needed.

.NET consumers use the same native OfficeIMO owner directly; installing Mailozaurr brings the matching OfficeIMO.Email dependency:

```csharp
using OfficeIMO.Email.Data;
using OfficeIMO.Email.Store;

using EmailDataOpenResult data = EmailDataArtifact.Open("offline.ost");
EmailStoreSession store = data.Store ?? throw new InvalidDataException("Not a mail store.");
foreach (EmailStoreFolderInfo folder in store.Folders) {
    Console.WriteLine($"{folder.Name}: {folder.ItemCount}");
}
```

Store exports support EML, MSG, OFT, TNEF, Mbox, Maildir, and EMLX. Conversion and merge commands create new Unicode PST files, leave every source read-only, and retain OfficeIMO's verification and diagnostic reports:

```powershell
ConvertTo-MailPst './offline.ost' './portable.pst'
Merge-MailStore './old.pst','./portable.pst' './combined.pst'
```

Both write commands accept `-StoreReaderOptions` for custom safety bounds and password-protected PST input. `Merge-MailStore` accepts one shared options object or one object per input path, so archives with different passwords or limits remain explicit.

Use `-Flatten` for directory exports when preserving a deep archive hierarchy would exceed the host's path limit, especially under Windows PowerShell 5.1.

OST input is read-only: Mailozaurr does not pretend to reconstruct server-only content or write an OST. OfficeIMO.Email also provides Unicode PST mutation, recovery export, compaction, splitting, associated Outlook data, reminder queues, and category/view/rule inspection. PowerShell callers can use those specialized APIs through the native `$data.Store` session; Mailozaurr does not duplicate their implementation.

`Import-MailData` is also the discovery entry point for standalone ICS and VCF files and Outlook Offline Address Book data. It returns the native OfficeIMO owner through `Calendar`, `Contact`, or `AddressBook`, preserving the complete API instead of projecting a smaller Mailozaurr copy.

The PowerShell module also offers direct read-only entry points for OfficeIMO.Email analysis:

| Command | Result |
| --- | --- |
| `Get-MailSemanticFingerprint` | Versioned semantic digest for grouping likely duplicates |
| `Compare-MailDataSemantic` | Native difference report for two imported emails |
| `Get-MailStoreConversation` | Bounded conversation graph with link confidence |
| `Get-MailStoreMaintenancePlan` | Source-bound validation and recovery recommendations |
| `Search-MailAddressBook` | Bounded OAB search report with a resume checkpoint |

```powershell
$source = Import-MailData './source.eml' -UseStreamingEmailReader
$copy = Import-MailData './copy.eml' -UseStreamingEmailReader
try {
    $source | Get-MailSemanticFingerprint
    $source | Compare-MailDataSemantic -ReferenceObject $copy
} finally {
    $source, $copy | Close-MailData
}
```

For duplicate grouping, compare fingerprints produced with the same schema version, algorithm, profile, and options. Use keyed `EmailSemanticComparisonOptions` before persisting fingerprints of private mail. Keep imported results open until comparison completes.

```powershell
$store = Import-MailData './archive.pst'
try {
    $store | Get-MailStoreConversation
    $store | Get-MailStoreMaintenancePlan -MaxItems 50000
} finally {
    $store | Close-MailData
}

$directory = Import-MailData './directory.oab'
try {
    $batch = $directory | Search-MailAddressBook -Term 'Ada' -MaxEntriesScanned 1000
    if ($batch.NextCheckpoint) {
        $directory | Search-MailAddressBook -Term 'Ada' -MaxEntriesScanned 1000 -ResumeFrom $batch.NextCheckpoint
    }
} finally {
    $directory | Close-MailData
}
```
...
- [Configuration and Usage](Docs/Configuration-and-Usage.md) - profile model, PowerShell setup, provider guidance, CLI recipes, and MCP usage
...
PowerShell 5.1 and PowerShell 7 can use that same profile layer through `Get/New/Set/Remove/Test-MailProfile`, protected `Set/Remove-MailProfileSecret`, and native JMAP discovery, mailbox, email, change, thread, and identity cmdlets. See [Configuration and Usage](Docs/Configuration-and-Usage.md#powershell-profile-and-jmap-usage) for a secure setup example.
...
- `provider ...` for Graph, Gmail, and JMAP provider-native operations
...
Connection-test JSON includes ordered, timed stages for profile resolution, session acquisition, and the requested provider probe. Typed `Evidence` reports non-secret protocol capabilities, authenticated identity, permission provenance, mailbox counts/cursors, and non-destructive send readiness when those facts are actually observable. Graph identity is verified with a live users-endpoint request; Gmail identity and mailbox totals come from `users.getProfile`; opaque permissions remain explicitly unavailable instead of being guessed. POP3 profiles participate in the same application surface with a virtual `INBOX` and normalized `uid:` message identifiers (falling back to a content-verified `hash:<sha256>:<occurrence>` identity when UIDL is unavailable).
...
Archive a selected inventory of provider messages as exact EML bytes:

```powershell
# One provider message ID per line; retain this file to resume the same snapshot.
mailozaurr mail archive-eml --profile work-imap --folder Inbox `
  --ids-file ./inbox-ids.txt --path ./inbox-archive --json
```

The archive records the selected IDs, profile identity, and verified provider mailbox scope, writes EML files in `eml/`, and keeps per-message SHA-256 records in `.mailozaurr-archive/`. Repeating the command with the same inventory verifies each retained file before skipping it, retries missing or altered files, and reports failed IDs. IMAP resumes bind to the authenticated server account, resolved folder, and folder UIDVALIDITY. POP3 resumes bind to the authenticated server account. Password and OAuth token rotation therefore preserve a valid resume when the authenticated account remains the same, while a server, user, folder epoch, or provider identity change stops the archive. Graph and Gmail verify the selected mailbox through their identity endpoints; if that lookup is denied or the identity changes, the archive stops. Equivalent mailbox and folder aliases do not invalidate the archive. The command accepts more than 1,000 IDs by processing bounded export batches while reusing the verified remote identity for the retained session. The ID file is the caller's selected inventory; this command does not discover every message in a mailbox. IMAP retrieves messages in bounded partial fetches. MailKit buffers each POP3 message before returning its stream, with transfer progress enforcing the configured size limit. Graph and Gmail provider APIs currently return buffered MIME payloads.
...
Queue processing records provider acceptance before removing the pending item. If removal fails, the next run retries local cleanup without sending the message again. `Send-EmailPendingMessage -ProcessAll` and a selected `-MessageId` can bypass the retry schedule, but they respect an active processing lease. If the provider accepts a message and both recording that acceptance and removing the item fail, delivery is indeterminate; inspect the provider before retrying it manually.

When upgrading workers that share a pending-message log, let in-flight sends finish, then stop every queue processor, including scheduled and forced PowerShell runs, before updating them. Restart processing after all workers use the new version; existing queued records remain readable. Forced retries can process pre-upgrade scheduled records immediately after restart. Do not run an older worker against a log written by the new version: older compaction can discard acceptance and lease markers, and older forced retries can ignore active leases.
...
- `Sources/Mailozaurr.Examples/AcquireGoogleTokenInteractive.cs` – C# sample for `GmailOAuthHelpers.AcquireTokenInteractiveAsync`.
